...
def cluster_kiosk_candidates(
    ...
    # HARDENING: reduce every candidate to its centroid, computed
    # AFTER reprojecting to the local metric CRS -- not before,
    # since centroid math on raw lat/lon geometry distorts anything
    # that isn't already a point. Raw OSM features aren't all
    # points (a market or school can be a polygon), and shapely's
    # .distance() has different semantics for a polygon than a
    # point (e.g. it's 0 if a point falls inside the polygon),
    # which the clustering and reporting logic both otherwise
    # assume is a simple point-to-point distance.
    local = result.to_crs(local_crs).copy()
    local["geometry"] = local.geometry.centroid
    result["geometry"] = local.geometry.to_crs(result.crs)
...
def analyze_store(
    ...
    board_placement_method = None

    if not board_candidates.empty:

        # HARDENING: candidate clustering is the preferred, more
        # granular path, but if it ever fails compactness validation
        # despite the fixes above -- an edge case not fully ruled
        # out -- fall back to the always-safe zone-level approach
        # instead of crashing the whole store analysis. A degraded
        # (coarser) result beats no result.
        try:

            clustered_candidates = cluster_no_parking_candidates(
                board_candidates,
                store,
                cluster_radius_m=150
            )
            ...
            print(
                f"  No Parking Board candidate clustering failed "
                f"({exc!r}) -- falling back to catchment-zone "
                f"allocation for this store."
            )

    if board_placement_method is None:
...
def build_store_map(full_result, zoom_start=14):
    ...
    m = folium.Map(
        ...
        # CONSOLIDATION FIX: cartodbpositron now warns that CartoDB
        # requires an API key going forward -- switched to plain
        # OpenStreetMap tiles, which need no key and carry no such
        # risk of quietly breaking later.
        tiles="OpenStreetMap"
    )
...
def build_store_onepager(result, precomputed_map=None):
    """
    ...
    precomputed_map: optional. If a caller (e.g. generate_activity())
    has already built the map via build_store_map(), pass it here to
    reuse it instead of building a second one from scratch. Existing
    callers that don't pass this get exactly the same behaviour as
    before -- a map is still built internally.
    """
    ...
    # -----------------------------------------------------
    # STEP B: use the supplied map, or build one if none was given
    # (this is the only behavioural change, and only when the new
    # parameter is actually used)
    # -----------------------------------------------------

    store_map = precomputed_map if precomputed_map is not None else build_store_map(full)
...
# =========================================================
# ACTIVITY GENERATION LAYER
# =========================================================
#
# Orchestration layer added on top of the existing engine. Does not
# change any analytical logic -- run_store(), build_store_map(),
# build_store_onepager(), and build_vendor_sheet() are unchanged and
# remain fully usable exactly as before. This layer exists so a
# future single Streamlit action can call one function and receive
# everything needed to display or save an activity's outputs,
# without app.py assembling pieces itself.
#
# generate_activity() calls run_store() exactly once and builds the
# vendor sheet and map from that single result -- no repeated
# analysis, no extra OSM calls. It does not call
# build_store_onepager() (that function already builds its own map
# internally, so including it here would mean building the map
# twice for no benefit -- see the documentation notes for this
# tradeoff).
...
class ActivityResult:
    """
    Everything generate_activity() produces for one store activity.

    activity_id:        human-readable ID (caller-provided or
                         generated -- see _generate_activity_id)
    metadata:            dict describing the request itself (see
                         generate_activity docstring) -- contains no
                         business performance data (sales, footfall,
                         ROI, etc.), only what was asked for
    analytical_result:   exactly what run_store(..., return_full_result=True)
                         returns -- a dict with "management_report"
                         and "full_result" keys
    management_report:   the same dict as
                         analytical_result["management_report"],
                         exposed directly for convenience
    management_report_html: ready-to-serve HTML string, exactly what
                         build_store_onepager() returns -- the actual
                         "management ready report" (tables + reasoning
                         + map in one file). Built from the same map
                         as the `map` field below, not a second one.
    vendor_sheet:        pandas DataFrame, exactly what
                         build_vendor_sheet() returns
    map:                 folium.Map object, exactly what
                         build_store_map() returns (not HTML -- call
                         .save(path) or ._repr_html_() as the
                         existing app.py already does)
    """
...
def _generate_activity_id():
    """
    Human-readable activity ID that doesn't require persistent
    storage or a global counter (which would reset on every
    Streamlit restart, and could collide across concurrent
    sessions). Year plus a short random suffix -- reads like
    OOH-2026-3F9A21C8 rather than a true sequential number, which
    would need a database this layer deliberately doesn't add yet.
    """
    import datetime
    import uuid
...
def generate_activity(
    ...
    """
    Orchestration layer around the existing analytical engine:
    run_store() once, then build the existing vendor sheet and map
    from that single result, packaged with activity metadata.

    Does not change scoring, allocation, catchment, or taxonomy
    logic -- it calls the existing functions unmodified. Coordinate
    and media-quantity validation is not duplicated here; it's
    reused from create_store_input() via the run_store() call below,
    which raises ValueError on bad input exactly as it already does
    today. This function additionally validates the new fields it
    introduces (search_radius, budget, go_live_date) before doing
    any expensive work.

    store_name vs city: the existing engine has a single "city"
    label used throughout reports, maps, and filenames. If
    store_name is given, it's used as that label (since it's
    typically the more specific value, e.g. an exact store address);
    otherwise city is used, matching every existing call site.
    Both values are still recorded separately in metadata regardless
    of which one was used as the label.

    Raises ValueError for invalid input (new fields here, existing
    fields via create_store_input inside run_store), and
    RuntimeError naming which specific step failed if the analytical
    run succeeds but a downstream output cannot be built.
    """
    import datetime

    # -----------------------------------------------------
    # Validate the fields this layer introduces. city / latitude /
    # longitude / media quantities are validated by the existing
    # create_store_input() when run_store() is called below -- not
    # duplicated here, per the reuse-existing-validation requirement.
    # -----------------------------------------------------

    label = store_name if (store_name and str(store_name).strip()) else city

    if not label or not str(label).strip():
        raise ValueError("Either city or store_name must be provided.")

    if latitude is None or longitude is None:
        raise ValueError("latitude and longitude are required.")

    if search_radius is None:
        raise ValueError("search_radius is required.")

    if float(search_radius) <= 0:
        raise ValueError("search_radius must be a positive number.")
    ...
    if go_live_date is not None and not isinstance(
        go_live_date, (str, datetime.date, datetime.datetime)
    ):
        raise ValueError(
            "go_live_date must be a string or a date/datetime object."
        )

    # -----------------------------------------------------
    # Run the existing analytical engine -- exactly once. Everything
    # below is built from this single result; nothing here triggers
    # another OSM call or re-runs the analysis.
    # -----------------------------------------------------

    print(f"generate_activity: starting analysis for '{label}'")

    try:
        analytical_result = run_store(
            city=label,
            latitude=latitude,
            longitude=longitude,
            auto_tops=auto_tops,
        ...
        raise RuntimeError(
            f"Analytical engine failed for '{label}': {exc}"
        ) from exc

    try:
        management_report = analytical_result["management_report"]
    except Exception as exc:
        raise RuntimeError(
            f"Analytical run succeeded but its management report "
            f"could not be read: {exc}"
        ) from exc

    # -----------------------------------------------------
    # Build the remaining existing outputs from that one result
    # -----------------------------------------------------

    try:
        vendor_sheet = build_vendor_sheet(analytical_result)
    except Exception as exc:
        raise RuntimeError(
            f"Analytical run succeeded but vendor sheet generation "
            f"failed: {exc}"
        ) from exc

    try:
        activity_map = build_store_map(analytical_result["full_result"])
    except Exception as exc:
        raise RuntimeError(
            f"Analytical run succeeded but map generation failed: {exc}"
        ) from exc

    try:
        # Reuses activity_map above rather than letting
        # build_store_onepager build its own -- one map per
        # activity, not two.
        management_report_html = build_store_onepager(
            analytical_result, precomputed_map=activity_map
        )
    except Exception as exc:
        raise RuntimeError(
            f"Analytical run succeeded but the management report "
            f"HTML could not be built: {exc}"
        ) from exc

    print(f"generate_activity: all outputs built for '{label}'")

    # -----------------------------------------------------
    # Activity ID + metadata. No business performance data (sales,
    # footfall, ROI, etc.) is recorded here -- only what was asked
    # for.
    # -----------------------------------------------------

    resolved_activity_id = (
        activity_id if (activity_id and str(activity_id).strip())
        else _generate_activity_id()
    )

    go_live_display = go_live_date
    if isinstance(go_live_date, (datetime.date, datetime.datetime)):
        go_live_display = go_live_date.isoformat()
    ...
    return ActivityResult(
        ...
        management_report=management_report,
        management_report_html=management_report_html,
        vendor_sheet=vendor_sheet,
        map=activity_map
    )


def save_activity_outputs(activity_result, output_dir="."):
    """
    Optional helper -- writes an activity's outputs to disk using
    its activity_id as the filename prefix. Not required for app.py
    to work; app.py can use ActivityResult's fields directly (e.g.
    via st.download_button) without ever calling this.

    The management report is three separate tables (auto_tops,
    pole_kiosks, no_parking_boards), not one flat file, so each is
    written separately rather than inventing a merged format that
    doesn't match its actual shape.

    Writes:
        <output_dir>/<activity_id>_management_report.html   (the actual
            management-ready report -- tables + reasoning + map, one file)
        <output_dir>/<activity_id>_management_report_auto_tops.csv
        <output_dir>/<activity_id>_management_report_pole_kiosks.csv
        <output_dir>/<activity_id>_management_report_no_parking_boards.csv
        <output_dir>/<activity_id>_vendor_sheet.csv
        <output_dir>/<activity_id>_map.html

    Returns a dict of {output_name: file_path} for what was written.
    """
    import os

    os.makedirs(output_dir, exist_ok=True)
    prefix = activity_result.activity_id
    paths = {}

    report_html_path = os.path.join(output_dir, f"{prefix}_management_report.html")
    with open(report_html_path, "w") as f:
        f.write(activity_result.management_report_html)
    paths["management_report_html"] = report_html_path

    for media_key, df in activity_result.management_report["recommendations"].items():
        path = os.path.join(
            output_dir, f"{prefix}_management_report_{media_key}.csv"
        )
        df.to_csv(path, index=False)
        paths[f"management_report_{media_key}"] = path

    vendor_path = os.path.join(output_dir, f"{prefix}_vendor_sheet.csv")
    activity_result.vendor_sheet.to_csv(vendor_path, index=False)
    paths["vendor_sheet"] = vendor_path

    map_path = os.path.join(output_dir, f"{prefix}_map.html")
    activity_result.map.save(map_path)
    paths["map"] = map_path
...
# =========================================================
#
# Runs generate_activity() once per row of a store spreadsheet,
# collecting successes and failures separately so one bad row
# doesn't stop the rest of the batch. Same principle as the single-
# activity layer: this only orchestrates the existing engine, it
# doesn't add or change any analytical logic. No Google Sheets/Drive
# involved -- input is an uploaded file, output is an in-memory ZIP
# for a direct download, per how this project delivers files.
...
_BATCH_COLUMN_ALIASES = {
    ...
    "store_code": ["store code", "store_code", "code", "id"],
    "auto_tops": ["auto tops", "auto_tops", "autotops"],
    "pole_kiosks": ["pole kiosks", "pole_kiosks", "pole kiosk", "kiosks"],
    "no_parking_boards": [
        "no parking boards", "no_parking_boards", "no parking board",
        "boards", "no parking"
    ],
    "search_radius": ["search radius", "search_radius", "radius"],
    "budget": ["budget"],
    "go_live_date": ["go live date", "go_live_date", "golive date", "go-live date"],
}


def _normalize_batch_columns(df):
    """
    Matches real-world spreadsheet column naming variation (case,
    spacing, underscores) against the canonical field names
    generate_activity() needs, without requiring the caller to match
    an exact header. Raises ValueError naming exactly which required
    column couldn't be found, rather than failing on a cryptic
    KeyError deep inside the batch loop.
    """
    normalized = {
    ...
    resolved = {}
    for canonical, aliases in _BATCH_COLUMN_ALIASES.items():
    ...
    required = [
        ...
        "auto_tops", "pole_kiosks", "no_parking_boards"
    ]
...
def run_batch_activities(
    stores_df,
    search_radius=3000,
    progress_callback=None
):
    """
    Run generate_activity() for every row in stores_df -- calls the
    analytical engine once per store, exactly as a single
    generate_activity() call would, just looped. One bad row (bad
    coordinates, an OSM failure, anything) does not stop the rest of
    the batch.

    Expected columns (case/spacing-insensitive -- see
    _BATCH_COLUMN_ALIASES for accepted variants):
        Store Name, Lat, Long, Store Code (optional -- used as the
        activity_id if given, so your own store codes carry through
        instead of a generated ID), Auto Tops, Pole Kiosks,
        No Parking Boards, and optionally Search Radius, Budget,
        Go Live Date (per-row values override the search_radius
        argument if that column is present).

    progress_callback: optional callable(index, total, store_name,
        status, error=None), called after each row completes --
        status is "success" or "failed". Purely for UI progress
        reporting; the batch runs identically with or without it.

    Returns (successes, failures):
        successes: list of ActivityResult
        failures:  list of {"row": <spreadsheet row number>,
                             "store_name": str, "error": str}
    """
    columns = _normalize_batch_columns(stores_df)

    successes = []
    failures = []
    total = len(stores_df)

    for position, (_, row) in enumerate(stores_df.iterrows(), start=1):

        store_name = row[columns["store_name"]]

        try:
            row_radius = search_radius
            if "search_radius" in columns and pd.notna(row[columns["search_radius"]]):
                row_radius = row[columns["search_radius"]]

            row_budget = None
            if "budget" in columns and pd.notna(row[columns["budget"]]):
                row_budget = row[columns["budget"]]

            row_go_live = None
            if "go_live_date" in columns and pd.notna(row[columns["go_live_date"]]):
                row_go_live = row[columns["go_live_date"]]

            row_activity_id = None
            if "store_code" in columns and pd.notna(row[columns["store_code"]]):
                row_activity_id = str(row[columns["store_code"]])

            print(f"generate_activity: batch row {position}/{total} -- '{store_name}'")

            activity = generate_activity(
                city=str(store_name),
                latitude=float(row[columns["latitude"]]),
                longitude=float(row[columns["longitude"]]),
                search_radius=row_radius,
                auto_tops=int(row[columns["auto_tops"]]),
                pole_kiosks=int(row[columns["pole_kiosks"]]),
                no_parking_boards=int(row[columns["no_parking_boards"]]),
                activity_id=row_activity_id,
                budget=row_budget,
                go_live_date=row_go_live
            )

            successes.append(activity)

            if progress_callback is not None:
                progress_callback(position, total, str(store_name), "success")
            ...
            print(f"  Row {position} ('{store_name}') failed: {exc!r}")

            if progress_callback is not None:
                progress_callback(position, total, str(store_name), "failed", str(exc))

    return successes, failures


def build_batch_zip(activity_results):
    """
    Package every ActivityResult's outputs into a single in-memory
    ZIP -- one folder per activity, named by activity_id, each
    containing the management report HTML, the vendor sheet CSV,
    and the full standalone map HTML. Returns raw zip bytes, ready
    for a Streamlit download button. Nothing is written to disk.
    """
    import io
    import zipfile
    ...
    with zipfile.ZipFile(buffer, "w", zipfile.ZIP_DEFLATED) as zf:
        for activity in activity_results:
            folder = activity.activity_id
            zf.writestr(
                f"{folder}/management_report.html",
                activity.management_report_html
            )
            zf.writestr(
                f"{folder}/vendor_sheet.csv",
                activity.vendor_sheet.to_csv(index=False)
            )
            zf.writestr(
                f"{folder}/map.html",
                activity.map.get_root().render()
            )
